...
left join
    (
    select ch.HISTORY_ID, ch.CAR_ID, datediff(h.END_DATE, h.START_DATE)+1 as DIFF ,
        case when 0 <= cp.DISCOUNT_RATE then cp.DISCOUNT_RATE else 0 end as DISCOUNT_RATE
    ...
    left join 
            ...
            from CAR_RENTAL_COMPANY_RENTAL_HISTORY _h) ch
